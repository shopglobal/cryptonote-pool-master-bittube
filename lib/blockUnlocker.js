    ...
    async.waterfall([
        ...
        function(blocks, callback){
            async.filter(blocks, function(block, mapCback){
                apiInterfaces.rpcDaemon('getblockheaderbyheight', {height: block.height}, function(error, result){
					...
					block.major_version = blockHeader.major_version;
                    block.minor_version = blockHeader.minor_version;
        ...
        function(blocks, callback){
            ...
            blocks.forEach(function(block){
				...
				if (block.major_version && block.major_version >= 4) {
                    log('info', logSystem, 'Block %d base BitTube dev & marketing share %d percent: %d', [block.height, 3, Math.round(block.reward * 0.03)]);
                    feePercent += 0.03;
                }

                if (block.major_version && block.major_version >= 5) {
                    log('info', logSystem, 'Block %d base BitTube airtime share %d percent: %d', [block.height, 27, Math.round(block.reward * 0.27)]);
                    feePercent += 0.27;
                }
