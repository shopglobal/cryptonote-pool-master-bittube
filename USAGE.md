...
"blockUnlocker": {
    ...
    "coreDevDonation": 0.1 //0.1% donation to send to core devs - works with Bytecoin, Monero, Dashcoin, QuarazCoin, Fantoncoin, AEON, Ryo, OneEvilCoin
},
...
First make sure that the ryod and ryo-wallet-rpc daemons are running:

```bash
./ryod --detach
./ryo-wallet-rpc --wallet-file=<wallet name> --rpc-bind-port <wallet port>
```

When both are running it's worth to make sure the ryod has it's blockchain
synced. If this is not the case your pool will show errors with "Core is busy"
until ryod is synchronized.
...
Ryo does have a testnet. Call daemon and simplewallet with --tesnet to connect to it.
Downloading the testnet blockchain may still take a while to start usint testnet, so you can use this excellent
 Monero tutorial, which also applies to Ryo, at http://moneroexamples.github.io/private-testnet/ to set up a private testnet.
